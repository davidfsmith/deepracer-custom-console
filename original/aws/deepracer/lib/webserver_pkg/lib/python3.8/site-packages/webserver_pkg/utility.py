...
from webserver_pkg.constants import SERVICE_CALL_TIMEOUT, SERVICE_CALL_SLEEP_TIME, SERVICE_CALL_RETRY_ATTEMPTS


def call_service_sync(cli, req, timeout=SERVICE_CALL_TIMEOUT, sleep_time=SERVICE_CALL_SLEEP_TIME):
    ...
    """
    attempt_number = 1
    while(attempt_number <= SERVICE_CALL_RETRY_ATTEMPTS):
        webserver_node = webserver_publisher_node.get_webserver_node()
        if cli.service_is_ready():
            webserver_node.get_logger().info(f"Service call initiated: {cli.srv_name} Attempt number: {attempt_number}")
            future = cli.call_async(req)
            sequence = -1
            pending_req = cli._pending_requests.copy()
            for seq, req_future in pending_req.items():
                if req_future == future:
                    sequence = seq
                    break        
            webserver_node.get_logger().info(f"New request: {sequence} {cli.srv_name} Attempt number: {attempt_number}")
            # checker for timeout
            elapsed_time = 0
            # flag to check if we should retry
            shouldRetry = False
            while not future.done():
                time.sleep(sleep_time)
                elapsed_time += sleep_time
                # after the service calls timeouts
                if elapsed_time >= timeout:
                    webserver_node.get_logger().error(f"Service call not completed before timeout: {cli.srv_name} Attempt number: {attempt_number}")
                    future.cancel()
                    # increment attempt number and retry the call again
                    attempt_number += 1
                    shouldRetry = True
                    break

            # checking if it timed out and retrying the call again
            if(shouldRetry):
                webserver_node.get_logger().info(f"Retrying service call: {cli.srv_name} Attempt number: {attempt_number}")
                continue
            if future.exception() is not None:
                webserver_node.get_logger().error(f"Error while calling service: {sequence} - "
                                                f"{cli.srv_name} - {future.exception()}")
            # if it reaches here, means the service call is successful
            webserver_node.get_logger().info(f"Service call finished: {sequence} {cli.srv_name} Attempt number: {attempt_number}")
            return future.result()
        else:
            webserver_node.get_logger().info(f"Service is not ready: {cli.srv_name}")
            return None
    # retries exhausted, so return None
    webserver_node.get_logger().error(f"Service call failed: {cli.srv_name} No retries left.")
    return None
