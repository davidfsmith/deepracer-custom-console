...
SERVICE_CALL_TIMEOUT = 10 # duration after we declare timeout after service is called
SERVICE_CALL_SLEEP_TIME = 0.01 # sleep time after which to periodically monitor if we have a response
SERVICE_CALL_RETRY_ATTEMPTS = 3 # retry attempts to use by default for each service
